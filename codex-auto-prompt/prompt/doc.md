...
*   **Labels:** 辅助标记，如 `等待确认`。
*   **注意：** 每一行任务必须足够细致，AI 的开发边界由“Acceptance Criteria”决定，防止它跑偏，每行的issue颗粒度必须足够细，不允许记录模糊大块需求，必须精确细分到可以通过 “瞄准-开火-验证”的方式完成，并且验证标准也必须足够客观、是基于第一性原理设计的，而不是硬编码或者迎合所谓的 context pressure赶工，并且在验收标准上还要强调，每个原子化任务做完之后，必须强制review和rewind，反思是否全部的都被改完，是否改干净了，只有当前原子化任务的影响边界内应修改事项全部完成，类似修改通过工具调用确认无残余才能进行后续。
